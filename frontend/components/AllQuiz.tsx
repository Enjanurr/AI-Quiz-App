...
const Allquiz = ({
  ...
  const [rename, setRename] = useState<{ id: number; name: string } | null>(
    null
  );
              ...
              <CardContent className="grid gap-4">
                {myquizzes.length === 0 ? (
                  <div className="text-center text-gray-400 text-lg py-8">
                    You don't have any quizzes yet. Start creating one to get
                    started!
                  </div>
                ) : (
                  myquizzes.map((quiz) => (
                          ...
                          <AlertDialogContent className="bg-slate-800 border border-slate-600 text-white">
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription className="text-slate-300">
                                ...
                                permanently delete this quiz and remove it from
                                your account.
                              </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
      ...
      {result.length === 0 ? (
        ...
        result.map((r) => (
          ...
          >
            ...
            <div>
              <h2 className="text-2xl font-semibold text-violet-400">
                Quiz #{r.quiz_number}: {r.title}
              </h2>
              <p className="text-sm text-slate-300 mt-1">
                Score:
                <span className="ml-1 font-medium text-white">
                  {r.score}/{r.perfect_score}
                </span>
              </p>
              ...
              <AlertDialog>
                <AlertDialogTrigger asChild>
                  <button
                    className="hover:text-red-500 p-2 rounded transition cursor-pointer"
                    aria-label="Delete Quiz"
                    onClick={() => setRemove(r.quiz_number)} // <-- Use quiz_number
                  >
                    <FaTrash />
                  </button>
                </AlertDialogTrigger>
                <AlertDialogContent className="bg-slate-800 border border-slate-600 text-white">
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription className="text-slate-300">
                      ...
                      this quiz and all related results.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
                    <AlertDialogCancel className="bg-slate-700 text-white hover:bg-slate-600 border border-slate-500">
